...
class AssessRequest(BaseModel):
    ...
    revenue: float
    expenses: float
    retainedEarnings: float | None = None   # If absent, proxied by equity
...
def _project_covenants(
    ...
    for i, cf in enumerate(forecasted_cfs):
        ...
        cash_shortfall = baseline_avg - cf  # positive = deficit vs baseline

        # DSCR: annualise monthly CF against annual debt service
        dscr: float | None = None
        if data.debtService and data.debtService > 0:
            dscr = round((cf * 12) / data.debtService, 4)

        # Quick Ratio: (CA - shortfall - inventory) / CL
        quick_ratio: float | None = None
        if cl > 0:
            quick_ratio = round((data.currentAssets - cash_shortfall - inventory) / cl, 4)

        # Current Ratio: (CA - shortfall) / CL
...
@app.get("/")
def health_check():
    return {"status": "FinSight API is running", "model": "DES (Double Exponential Smoothing)"}
